...
newtype Parser r e a = Parser {runParser# :: r -> Addr# -> Addr# -> Res# e a}


instance Functor (Parser r e) where
  fmap f (Parser g) = Parser \r eob s -> case g r eob s of
    OK# a s -> let !b = f a in OK# b s
    x       -> unsafeCoerce# x
  {-# inline fmap #-}

  (<$) a' (Parser g) = Parser \r eob s -> case g r eob s of
    OK# a s -> OK# a' s
    x       -> unsafeCoerce# x
  {-# inline (<$) #-}

err :: e -> Parser r e a
err e = Parser \r eob s -> Err# e
{-# inline err #-}

empty :: Parser r e a
empty = Parser \r eob s -> Fail#
{-# inline empty #-}

ask :: Parser r e r
ask = Parser \r eob s -> OK# r s
{-# inline ask #-}

local :: (r' -> r) -> Parser r e a -> Parser r' e a
local f (Parser g) = Parser \r eob s -> let!r' = f r in g r' eob s
{-# inline local #-}
...
instance Applicative (Parser r e) where
  ...
  {-# inline pure #-}
  Parser ff <*> Parser fa = Parser \r eob s -> case ff r eob s of
    OK# f s -> case fa r eob s of
      OK# a s  -> let !b = f a in OK# b s
      x        -> unsafeCoerce# x
    x -> unsafeCoerce# x
  {-# inline (<*>) #-}
  Parser fa <* Parser fb = Parser \r eob s -> case fa r eob s of
    OK# a s   -> case fb r eob s of
      OK# b s -> OK# a s
      x -> unsafeCoerce# x
    x -> unsafeCoerce# x
  {-# inline (<*) #-}
...
instance Monad (Parser r e) where
  return = pure
  {-# inline return #-}
  Parser fa >>= f = Parser \r eob s -> case fa r eob s of
    OK# a s -> runParser# (f a) r eob s
    x       -> unsafeCoerce# x
  {-# inline (>>=) #-}
  Parser fa >> Parser fb = Parser \r eob s -> case fa r eob s of
    OK# a s -> fb r eob s
...
eof :: Parser r e ()
eof = Parser \r eob s -> case eqAddr# eob s of
  1# -> OK# () s
  _  -> Fail#
{-# inline eof #-}
...
(<|>) (Parser f) (Parser g) = Parser \r eob s ->
  case f r eob s of
    Fail# -> g r eob s
    x     -> x
{-# inline (<|>) #-}
...
try (Parser f) = Parser \r eob s -> case f r eob s of
  Err# _ -> Fail#
  x      -> x
{-# inline try #-}
...
cut (Parser f) e = Parser \r eob s -> case f r eob s of
  Fail# -> Err# e
  x     -> x
{-# inline cut #-}
...
optioned (Parser f) just (Parser nothing) = Parser \r eob s -> case f r eob s of
  OK# a s -> runParser# (just a) r eob s
  Fail#   -> nothing r eob s
  Err# e  -> Err# e
{-# inline optioned #-}

-- | Parse any `Char` in the ASCII range.
anyCharA :: Parser r e Char
anyCharA = Parser \r eob buf -> case eqAddr# eob buf of
  1# -> Fail#
...
anyChar = Parser \r eob buf -> case eqAddr# eob buf of
  1# -> Fail#
  _  -> case derefChar8# buf of
    c1 -> case c1 `leChar#` '\x7F'# of
      1# -> OK# (C# c1) (plusAddr# buf 1#)
...
-- | Skip any `Char`.
anyChar_ :: Parser r e ()
anyChar_ = Parser \r eob buf -> case eqAddr# eob buf of
  1# -> Fail#
  _  -> case derefChar8# buf of
    c1 -> case c1 `leChar#` '\x7F'# of
      1# -> OK# () (plusAddr# buf 1#)
...
satisfyA f = Parser \r eob s -> case runParser# anyCharA r eob s of
  OK# c s | f c -> OK# c s
  _             -> Fail#
{-#  inline satisfyA #-}
...
satisfy' f1 f2 f3 f4 = Parser \r eob buf -> case eqAddr# eob buf of
  1# -> Fail#
  _  -> case derefChar8# buf of
    c1 -> case c1 `leChar#` '\x7F'# of
...
many_ (Parser f) = go where
  go = Parser \r eob s -> case f r eob s of
    OK# a s -> runParser# go r eob s
    Fail#   -> OK# () s
    Err# e  -> Err# e
{-# inline many_ #-}
...
br pa pt pf = Parser \r eob s -> case runParser# pa r eob s of
  OK# _ s -> runParser# pt r eob s
  Fail#   -> runParser# pf r eob s
  Err# e  -> Err# e
{-# inline br #-}
...
lookahead (Parser f) = Parser \r eob s ->
  case f r eob s of
    OK# a _ -> OK# a s
    x       -> x
{-# inline lookahead #-}
...
getPos = Parser \r eob s -> OK# (addr2Pos# eob s) s
{-# inline getPos #-}

-- | Set the current parsing position. Warning: this may crash if the input `Pos`
--   points outside the current buffer.
setPos :: Pos -> Parser r e ()
setPos s = Parser \r eob _ -> OK# () (pos2Addr# eob s)
{-# inline setPos #-}
...
spanOf (Parser f) = Parser \r eob s -> case f r eob s of
  OK# a s' -> OK# (Span (addr2Pos# eob s) (addr2Pos# eob s')) s'
  x        -> unsafeCoerce# x
{-# inline spanOf #-}
...
spanned (Parser f) g = Parser \r eob s -> case f r eob s of
  OK# a s' -> runParser# (g a (Span (addr2Pos# eob s) (addr2Pos# eob s'))) r eob s'
  x        -> unsafeCoerce# x
{-# inline spanned #-}

-- | Run a parser in a given input span. The state of the input is restored after the parser
--   finishes, so `inSpan` does not consume input and has no side effect.
--   Warning: this operation may crash if the given span points outside the current parsing
--   buffer. The validity of the span is not checked! It's always safe to use `inSpan` if the span
--   comes from a previous `spanned` usage.
inSpan :: Span -> Parser r e a -> Parser r e a
inSpan (Span s eob) (Parser f) = Parser \r eob' s' ->
  case f r (pos2Addr# eob' eob) (pos2Addr# eob' s)  of
    OK# a _ -> OK# a s'
    x       -> unsafeCoerce# x
{-# inline inSpan #-}
...
testParser :: Parser r e a -> r -> B.ByteString -> Result e a
testParser pa r s = runParser pa r s

packUTF8 :: String -> B.ByteString
packUTF8 = B.pack . concatMap charToBytes

testStrParser :: Parser r e a -> r -> String -> Result e a
testStrParser pa r s = runParser pa r (packUTF8 s)
...
--   invalid.
spanToByteString :: B.ByteString -> Span -> B.ByteString
spanToByteString bstr (Span (Pos n) (Pos n')) =
  B.take (n - n') (B.drop (B.length bstr - n) bstr)
{-# inline spanToByteString #-}

-- | Create a `ByteString` from a `Span`. The first argument is the buffer the
--   `Span` points into.
unsafeSpanToByteString :: B.ByteString -> Span -> B.ByteString
unsafeSpanToByteString bstr (Span (Pos n) (Pos n')) =
...
scanAny8# = Parser \r eob s -> OK# (W8# (indexWord8OffAddr# s 0#)) (plusAddr# s 1#)
{-# inline scanAny8# #-}

scan8# :: Word -> Parser r e ()
scan8# (W# c) = Parser \r eob s ->
...
scanPartial64# (I# len) (W# w) = Parser \r eob s ->
...
ensureBytes# (I# len) = Parser \r eob s ->
